...
const RollSnapshot = ({ savedRolls, setSavedRolls, purchaseData, setPurchaseData, dates, setDates, reserves, setReserves, currency, setCurrency, extras, setExtras, summonStats, setSummonStats, summonOdds, setSummonOdds, thisRoll }) => {
  ...
  const confirmEdit = () => {
    if (window.confirm('Load selected roll into the editing form?')) {
      ...
      setDates({ ...dates, target: new Date(rollData.bannerDate) });
      ...
      setSummonStats({ ...summonStats, targetName: rollData.targetName });
      ...
      <FormControl marginLeft="auto" marginRight="auto" onChange={handleFormUpdate}>
        <Grid w='100%' templateRows='repeat(1, 1fr)' templateColumns='repeat(10, 1fr)' p="6px" gap={5}>
          <GridItem rowSpan={1} colSpan={3}>
            <Select className="form-input" name="targetName" value={rollData.targetName} placeholder={'Target Servant'} mb="8px" >
            ...
            {
              editingDates === false
                ? <Input className="form-input" name="bannerDate" type="text" readOnly={true} onClick={dateRangeUpdate} value={rollData.bannerDate} />
          ...
          <GridItem rowSpan={2} colSpan={3}>
            <Grid w='100%' templateRows='repeat(1, 1fr)' templateColumns='repeat(8, 1fr)' gap={2} >
              ...
              <GridItem colSpan={7}>
                <Input className="form-input" name="sq" type="number" value={rollData.budget.sq} />
              </GridItem>
              <GridItem colSpan={1}>
                <FormLabel>Tickets:</FormLabel>
              </GridItem>
              <GridItem colSpan={7}>
                <Input className="form-input" name="tx" type="number" value={rollData.budget.tx} />
              </GridItem>
            </Grid>
          </GridItem>
